...
export default function AddressDisplay({ address, showIcon = true, iconSize = 20, className = '', link = true }) {
  if (!address) return <span className="text-gray-500">-</span>
  
  const displayAddress = `${address.substring(0, 8)}...${address.substring(address.length - 4)}`
